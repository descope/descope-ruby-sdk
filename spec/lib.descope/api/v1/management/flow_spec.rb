...
describe Descope::Api::V1::Management::Flow do
  ...
  context '.list_or_search_flows' do
    ...
    it 'is expected to get flows' do
      expect(@instance).to receive(:mgmt_post).with(FLOW_LIST_PATH, { ids: %w[123 456] })
  ...
  context '.export_flow' do
    ...
    it 'is expected to export flow' do
      expect(@instance).to receive(:mgmt_post).with(FLOW_EXPORT_PATH, { flowId: '123' })
  ...
  context '.export_theme' do
    ...
    it 'is expected to export theme' do
      expect(@instance).to receive(:mgmt_post).with(THEME_EXPORT_PATH)
  ...
  context '.import_theme' do
    ...
    it 'is expected to import theme' do
      expect(@instance).to receive(:mgmt_post).with(THEME_IMPORT_PATH, { theme: 'theme123' })
